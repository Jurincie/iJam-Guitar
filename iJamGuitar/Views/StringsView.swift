...
struct StringsView: View {
    ...
    var body: some View {
        ...
        .contentShape(Rectangle())
        .gesture(drag)
        .alert("Master Volume is OFF", isPresented: Bindable(appStates.first!).showVolumeAlert) {
            Button("OK", role: .cancel) { appStates.first!.showVolumeAlert = false }
        }
        .alert("Another App is using the Audio Player", isPresented: Bindable(appStates.first!).showAudioPlayerInUseAlert) {
            Button("OK", role: .cancel) { appStates.first!.showAudioPlayerInUseAlert = false }
        }
