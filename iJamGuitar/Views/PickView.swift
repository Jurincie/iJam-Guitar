...
extension PickView {
    func PickButton() -> some View {
        let button =  Button(action: {
            if appStates.first!.selectedChordIndex != pick.id || chordIsAltered(pick.id) {
    ...
    func makeChosenPicksChordActive() {
        ...
        if let chordNames = appStates.first!.activeChordGroup?.availableChordNames {
            ...
            if let newActiveChord = appStates.first!.getChord(name: newActiveChordName,
                                                   tuning: appStates.first!.activeTuning) {
                appStates.first!.activeChordGroup?.activeChord = newActiveChord
                appStates.first!.selectedChordIndex = self.pick.id
                UserDefaults.standard.setValue(appStates.first!.activeChordGroup?.activeChord?.name, forKey: "ActiveChordName")
            }
        }
    }
